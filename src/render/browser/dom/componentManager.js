...
ComponentManager.prototype = {
  ...
  getJudInstance () {
    if (!this._judInstance) {
      this._judInstance = global.jud
        ? global.jud.getInstance(this.id)
        : null
    }
    return this._judInstance
  },

  // Fire a event 'renderbegin'/'renderend' on body element.
  rendering () {
    function _renderingEnd () {
      // get jud instance root
  ...
  createBody (element) {
    ...
    const body = document.querySelector('#' + this.getJudInstance().rootId)
          || document.body
  ...
  createFinish (callback) {
    window.dispatchEvent(new Event('jud:createfinish'))
  },

  updateFinish (callback) {
    window.dispatchEvent(new Event('jud:updatefinish'))
  },

  refreshFinish (callback) {
    window.dispatchEvent(new Event('jud:refreshfinish'))
  }
